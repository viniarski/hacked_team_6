...
export async function POST(req) {
  ...
  try {
    ...
    try {
      ...
      } else if (plantData["Common name"] && Array.isArray(plantData["Common name"]) && plantData["Common name"].length > 0) {
        name = plantData["Common name"][0];
      }
      
      // Extract temperature values
      let temperature = null;
      if (plantData["Temperature min"] && plantData["Temperature min"]["C"] && 
          plantData["Temperature max"] && plantData["Temperature max"]["C"]) {
        const minTemp = plantData["Temperature min"]["C"];
        const maxTemp = plantData["Temperature max"]["C"];
        temperature = (minTemp + maxTemp) / 2;
      }
      
      // Extract brightness from light values
      let brightness = null;
