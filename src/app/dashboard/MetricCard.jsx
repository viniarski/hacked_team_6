...
export const MetricCard = ({ title, value, unit, icon: Icon, color, optimal, warning, critical, gradient }) => {
  ...
  return (
    <div
      className={`relative backdrop-blur-lg bg-[#2c392f]/40 rounded-2xl border border-[#4a5d4e] p-6 transition-all duration-300 ${
    ...
    >
      ...
      {isHovered && (
        <div className={`absolute inset-x-0 -bottom-0 h-1 rounded-b-2xl bg-gradient-to-r ${gradient}`} />
      )}
    </div>
  );
};